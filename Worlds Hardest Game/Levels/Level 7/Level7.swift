...
extension Level7 {
    override func update(_ currentTime: TimeInterval) {
        ...
        if (player!.position.x >= 120 && floorTrapHidden) {
            if (floorTrapHidden) {
                releaseFloorHiddenTrap()
            }
        } else if (player!.position.x >= 330 && wallTrapHidden) {
            if (wallTrapHidden) {
                pushOutWallTrap()
            }
        } else if (player!.position.x >= -60 && floorAlive) {
            if (floorAlive) {
                removeFloorTrap()
            }
        }
...
extension Level7 {
    ...
    func removeFloorTrap() {
        fakeFloor?.alpha = 0
        fakeFloor?.position.x = -120
        
        floorAlive = false
    }
    
    func resetTraps() {
        fakeFloor?.position.x = -20
        fakeFloor?.alpha = 1
        wallTrap?.alpha = 0
        hiddenTrap?.alpha = 0
...
extension Level7 {
    ...
    func showGameOver() {
        let gameOver = Level7(fileNamed: "Level7")
