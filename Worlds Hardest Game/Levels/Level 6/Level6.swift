...
extension Level6 {
    override func update(_ currentTime: TimeInterval) {
        ...
        if (player!.position.x >= 306 && floorTrapHidden) {
            if (floorTrapHidden) {
                releaseFloorHiddenTrap()
            }
        } else if (player!.position.x >= 100 && wallTrapHidden) {
            if (wallTrapHidden) {
                pushOutWallTrap()
            }
        } else if (player!.position.x >= -105 && floorAlive) {
            if (floorAlive) {
                removeFloorTrap()
            }
        }
...
extension Level6 {
    ...
    func showGameOver() {
        let gameOver = Level6(fileNamed: "Level6")
